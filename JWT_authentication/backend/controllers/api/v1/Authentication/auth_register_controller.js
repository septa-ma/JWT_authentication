...
module.exports = new class AuthLoginController extends controller {
    ...
    async registerHP(req, res){
        const { project_name, first_name, last_name, phone_number, national_code } = req.body;
    ...
    async register(req, res){
        const { project_name, first_name, last_name, phone_number, user_name } = req.body;
        ...
        let pass;
        let proType;
        if(( (project_name == 'gargot_app') || (project_name == 'gargot_web') || (project_name == 'nahalito_app') )) {  
            // Check available user information 
            const existUser = await this.model.user.findOne(existUserFilter);
            if ( existUser) {
                return res.status(400).json({ message: 'قبلا اطلاعات شما ثبت شده است' });
